...
clean_user_all() {
    ...
    echo -e "${CYAN}─────────────────────────────────────────────────────────────────${NC}"
    clean_dir "$user_home/Library/Caches" "General Caches" "$use_sudo"
    clean_dir "$user_home/Library/Caches/com.apple.Safari" "Safari" "$use_sudo"
    clean_dir "$user_home/Library/Caches/com.google.Chrome" "Chrome" "$use_sudo"
    clean_dir "$user_home/Library/Caches/Google/Chrome" "Chrome (Google)" "$use_sudo"
    clean_dir "$user_home/Library/Caches/com.microsoft.VSCode" "VS Code" "$use_sudo"
    clean_dir "$user_home/Library/Caches/com.spotify.client" "Spotify" "$use_sudo"
    clean_dir "$user_home/Library/Caches/com.tinyspeck.slackmacgap" "Slack" "$use_sudo"
    clean_dir "$user_home/Library/Caches/Yarn" "Yarn" "$use_sudo"
    clean_dir "$user_home/Library/Caches/Homebrew" "Homebrew" "$use_sudo"
    clean_dir "$user_home/Library/Caches/Cypress" "Cypress" "$use_sudo"
    clean_old_files "$user_home/Library/Logs" 7 "Logs (>7 days)" "$use_sudo"
    
    # Trash
    echo ""
    echo -e "${BOLD}${YELLOW}🗑️  Trash${NC}"
    echo -e "${CYAN}─────────────────────────────────────────────────────────────────${NC}"
    local trash_path="$user_home/.Trash"
    if [ -d "$trash_path" ]; then
        ...
        if [ -n "$size_before" ] && [ "$size_before" -gt 0 ]; then
            ...
            if [ "$use_sudo" = "false" ] && command -v osascript &> /dev/null; then
...
APPLESCRIPT
    ...
    # Xcode (if exists)
    if [ -d "$user_home/Library/Developer/Xcode" ] || [ -d "/Applications/Xcode.app" ]; then
        echo ""
        echo -e "${BOLD}${YELLOW}🍎 Xcode${NC}"
        echo -e "${CYAN}─────────────────────────────────────────────────────────────────${NC}"
        
        if [ -d "$user_home/Library/Developer/Xcode/DerivedData" ]; then
            clean_dir "$user_home/Library/Developer/Xcode/DerivedData" "DerivedData" "$use_sudo"
        fi
        
        if [ -d "$user_home/Library/Developer/Xcode/Archives" ]; then
            clean_old_files "$user_home/Library/Developer/Xcode/Archives" 30 "Archives (>30 days)" "$use_sudo"
        fi
        
        if [ -d "$user_home/Library/Caches/com.apple.dt.Xcode" ]; then
            clean_dir "$user_home/Library/Caches/com.apple.dt.Xcode" "Xcode Caches" "$use_sudo"
        fi
        
        if [ -d "$user_home/Library/Logs/DiagnosticReports" ]; then
            clean_old_files "$user_home/Library/Logs/DiagnosticReports" 7 "Diagnostic Reports (>7 days)" "$use_sudo"
        fi
        
        if [ -d "$user_home/Library/Developer/Xcode/iOS DeviceSupport" ]; then
            clean_old_files "$user_home/Library/Developer/Xcode/iOS DeviceSupport" 90 "DeviceSupport (>90 days)" "$use_sudo"
        fi
    fi
}
...
# ────────────────────────────────
# External Volumes Cleanup
# ────────────────────────────────

echo ""
echo -e "${BOLD}${CYAN}╔════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BOLD}${CYAN}║                                                                ║${NC}"
echo -e "${BOLD}${MAGENTA}║                   💾 EXTERNAL VOLUMES                          ║${NC}"
echo -e "${BOLD}${CYAN}║                                                                ║${NC}"
echo -e "${BOLD}${CYAN}╚════════════════════════════════════════════════════════════════╝${NC}"
echo ""
has_external_volumes=false
for volume in /Volumes/*; do
    if [ -d "$volume" ] && [ "$volume" != "/Volumes/Macintosh HD" ] && [ "$volume" != "/Volumes/Preboot" ]; then
        volume_trash="$volume/.Trashes"
        if [ -d "$volume_trash" ]; then
            has_external_volumes=true
            volume_name=$(basename "$volume")
            if [ "$SUDO_MODE" = "true" ]; then
                clean_dir "$volume_trash" "$volume_name" true
            else
                clean_dir "$volume_trash" "$volume_name" false
            fi
        fi
    fi
done
if [ "$has_external_volumes" = "false" ]; then
    echo -e "${YELLOW}  • No external volumes with trash found${NC}"
fi
...
echo -e "${CYAN}─────────────────────────────────────────────────────────────────${NC}"
clean_old_files "/private/var/log" 30 "Logs (>30 days)" "$SUDO_MODE"

echo ""
echo -e "${BOLD}${YELLOW}⏱️  Temporary Files${NC}"
echo -e "${CYAN}─────────────────────────────────────────────────────────────────${NC}"
clean_dir "/private/var/tmp" "Temporary Files" "$SUDO_MODE"
clean_dir "/private/var/folders" "Temporary Folders" "$SUDO_MODE"

echo ""
echo -e "${BOLD}${YELLOW}🍎 NSServices Cache${NC}"
echo -e "${CYAN}─────────────────────────────────────────────────────────────────${NC}"
if [ "$SUDO_MODE" = "true" ]; then
    for user_dir in /Users/*; do
        if [ -d "$user_dir" ] && [ "$user_dir" != "/Users/Shared" ]; then
            if [ -f "$user_dir/Library/Caches/com.apple.nsservicescache.plist" ]; then
                sudo rm -f "$user_dir/Library/Caches/com.apple.nsservicescache.plist" 2>/dev/null || true
            fi
        fi
    done
    echo -e "${GREEN}  ✓ NSServices Cache removed${NC}"
else
    if [ -f "$ORIGINAL_HOME/Library/Caches/com.apple.nsservicescache.plist" ]; then
        rm -f "$ORIGINAL_HOME/Library/Caches/com.apple.nsservicescache.plist" 2>/dev/null || true
        echo -e "${GREEN}  ✓ NSServices Cache removed${NC}"
    fi
fi
...
if command -v docker &> /dev/null; then
    ...
    if timeout 3 docker info &>/dev/null; then
        ...
        echo -e "${CYAN}     Start Docker Desktop if you want to clean Docker data${NC}"
    fi
fi
...
# Clean Docker data from all users (Docker.raw can take a lot of space)
if [ "$SUDO_MODE" = "true" ]; then
    echo ""
    ...
    for user_dir in /Users/*; do
        if [ -d "$user_dir" ] && [ "$user_dir" != "/Users/Shared" ]; then
            docker_data="$user_dir/Library/Containers/com.docker.docker"
...
echo ""
echo -e "${BOLD}${GREEN}🎉 All clean! Your Mac is lighter now.${NC}"
echo ""
